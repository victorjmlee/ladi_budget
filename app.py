...
def ocr_extract():
    ...
    try:
        ...
        try:
            ...
            # 한글 문자 사이 불필요 공백 제거 + 컬럼 구분 공백 보장
            def _normalize_ocr_line(line):
                s = line.rstrip()
                if not s.strip():
                    return ""
                # 1. 한글 사이 공백 1개 제거: "일 반 운영비" → "일반운영비"
                s = _re.sub(r"([가-힣])\s([가-힣])", r"\1\2", s)
                s = _re.sub(r"([가-힣])\s([가-힣])", r"\1\2", s)
                s = _re.sub(r"([가-힣])\s([가-힣])", r"\1\2", s)
                # 2. 쉼표 뒤 공백 제거 (금액 보호: "43, 936" → "43,936")
                s = _re.sub(r",\s+", ",", s)
                # 3. 컬럼 구분: 한글↔숫자 전환점에 2칸 공백 삽입
                s = _re.sub(r"([가-힣)）\]])\s+(\d)", r"\1  \2", s)
                s = _re.sub(r"(\d)\s+([가-힣(（\[])", r"\1  \2", s)
                # 4. 숫자↔숫자 사이 (금액 컬럼 분리)
